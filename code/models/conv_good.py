...
class ConvGood(models.basicmodel.BasicModel):
    ...
    def logits(self):
        ...
        i(tf.layers.dropout(o(), rate=0.10, training=is_training))
    ...
    @lazy_scope_property
    def l2_loss(self):
        return 1e-6 * tf.add_n([tf.nn.l2_loss(v) for v in tf.trainable_variables(self.scope) if 'bias' not in v.name])
