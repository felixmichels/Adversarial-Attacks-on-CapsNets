...
class DCNet2(models.basicmodel.BasicModel):
    """
    ...
    def encoder(self):
        ...
        i(tf.layers.conv2d(o(), filters=16-self.shape[-1], kernel_size=3, strides=1, padding='same', activation=tf.nn.relu))
        ...
        i(tc.layers.PrimaryConvCaps2D(kernel_size=5, types=32+self.garbage_class, dimensions=16, strides=2, data_format='channels_last'))
        i(o()[:,:,:,:32,:])
        ...
        width = self.shape[0]
        k = ((width - 4 + 1)//2 - 2 + 1) // 2
        i(tc.layers.ConvCaps2D(kernel_size=k, types=self.num_classes+self.garbage_class, dimensions=36, name="class-caps",
                               data_format='channels_last'))
