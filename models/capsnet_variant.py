...
class CapsNetVariant(models.basicmodel.BasicModel):
    """
    ...
    def encoder(self):
        ...
        i(tf.layers.conv2d(o(), filters=B, kernel_size=K, strides=1, activation=tf.nn.leaky_relu))
        i(tf.layers.batch_normalization(o(), training=is_training))

        tf.logging.debug('Shape after conv-layers: %s', o().get_shape())

        i(tc.layers.PrimaryConvCaps2D(kernel_size=9, types=C, dimensions=8, strides=2, data_format='channels_last'))

        tf.logging.debug('Shape after primary caps: %s', o().get_shape())

        ## only drop whole capsules
        # noise_shape = tf.concat([tf.shape(o())[:-1], [1]], axis=0)
        # i(tf.layers.dropout(o(), rate=0.1, noise_shape=noise_shape, training=is_training))

        # tf.logging.debug('Shape after dropout: %s', o().get_shape())
    ...
    def decoder(self):
        ...
        i(tf.layers.dense(o(), self.sizes['decoder_dense1'], activation=tf.nn.relu))
        i(tf.layers.dense(o(), self.sizes['decoder_dense2'], activation=tf.nn.relu))
