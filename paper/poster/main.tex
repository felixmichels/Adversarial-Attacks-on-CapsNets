				...
				\begin{column}{\onecolwid}\vspace{-.6in} % The second column within column 2 (column 2.2)
					\begin{block}{Transferability of Adversarial Examples}
					   ...
					   \begin{figure}
						...
						\draw[arrow](pertconv) to [bend right,looseness=1.2] node[above] {evaluate} (capsnet);
						\end{tikzpicture}}\caption{\phantom{T}Our evaluation procedure. The light orange arrows show the usual application of adversarial perturbations.}
					\end{figure}
				
					To quantify the transferability we evaluate the perturbations on the other architecture. The fooling rates corresponding to the (dark) orange arrows are shown in Tab.~\ref{tab:attacks}. Especially the smaller universal perturbations calculated on the ConvNet generalize well to the CapsNet (see also Tab.~\ref{tab:norms}).
