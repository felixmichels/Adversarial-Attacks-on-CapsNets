...
The low success rate of the transfer attacks and the radically different construction of the networks themselves however suggest that there may be some structural difference between adversarial examples computed for ConvNets and those computed for CapsNets.
...
\begin{wrapfigure}{R}{.5\textwidth}
	...
	\caption[t-SNE plot with additional ConvNet]{t-SNE plot of universal perturbations for CIFAR-10 with additional ConvNet architecture.}
	\label{fig:tsnealt}
\end{wrapfigure}
...
By including universal perturbations calculated for another ConvNet with a different architecture in \Cref{fig:tsnealt} we find the clusters of the two ConvNets still significantly separated from the cluster of the CapsNet.
...
For this purpose, let us denote with $\delta(x)$ the minimal adversarial perturbation for the input $x \in [0,1]^n$, and
\begin{equation*}
N = \begin{bmatrix}
