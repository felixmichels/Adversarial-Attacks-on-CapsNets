...
\begin{longtable*}{m{1.6cm}m{2.2cm}m{4.2cm}m{4.2cm}}
	...
	\begin{flushright} CW \end{flushright} & \includegraphics[height=2cm]{cifar10_carlini_wagner_orig.pdf} & \includegraphics[height=2cm]{cifar10_carlini_wagner_caps.pdf} & \includegraphics[height=2cm]{cifar10_carlini_wagner_conv.pdf}\\
	\\
	\begin{flushright} Boundary \end{flushright} &
	...
	\\
	\begin{flushright} DeepFool \end{flushright} &
	...
	\\
	\begin{flushright} Universal \end{flushright} &
...
\subsubsection{Singular Values of Adversarial Examples}
...
At first, it was believed that the cause of adversarial vulnerability is the somewhat discontinuous nature of neural networks, but \citet{fgsm} argued that linearity is at fault.
No bounds for the Lipschitz constant of the routing-by-agreement algorithm are known.
ConvNets with ReLU are actually piecewice linear, but CapsNet are not.
This is evidence, that CapsNet behave somewhat linear.
...
\begin{figure}
	...
	\caption[Singular values of adversarial examples]{Singular values of the matrix containing normal vectors to the decision boundary}
	\label{fig:svd}
\end{figure}
