...
An important problem to consider is the definition of \emph{closeness} to the original image. Possibilities for such an image similarity metric include simple $L^p$ norms (including $p=0$),
...
Here we will examine adversarial perturbations optimized towards a small $L^2$ norm, since it is often used in discussion about adversarial attacks and is a good compromise between the other commonly used, but more specific scenario targeting $L^0$ and $L^\infty$ norms.
