...
Throughout this paper we denote the input image as $x\in [0,1]^{n\times n}$, the neural network's output before the softmax activation as $F(x)$ and the perturbation as $\delta$. Furthermore, we refer to the $i$-th entry of $F(x)$ as $F(x)_i$.
